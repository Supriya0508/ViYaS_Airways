...
module.exports.search = function (from, to, date, Callback) {
    let query = `SELECT * FROM FLIGHT WHERE FROM_CITY = '${from}' AND TO_CITY = '${to}' AND DEP_DATE = '${date}';`;
...
module.exports.payment_initial = function (flightid) {
    let query = `INSERT INTO PAYMENT(FLIGHT_ID) VALUES(${flightid});`
...
function fary(flightid, userid, transid, time, mode, pname) {
    let query = `SELECT FARE FROM FLIGHT WHERE FLIGHT_ID=${flightid};`;
    conn.query(query, (err, rows) => {
        if (err) console.log(err);
...
function finale(flightid, userid, transid, fare, time, mode, pname) {
    let query = `UPDATE PAYMENT SET MODE='${mode}' ,USER_ID=${userid} ,FARE=${fare} ,TIME='${time}' WHERE TRANS_ID=${transid};`;
    conn.query(query, (err) => {
        if (err) console.log(err);
...
module.exports.disTicket = function (username, Callback) {
    q = `SELECT B.PNR_NUM,B.PASS_NAME,B.DATE,B.BOARD_TIME,B.FROM_CITY,B.TO_CITY,B.SEAT_NUMBER,P.FARE,B.STATUS FROM BOARDING_PASS B,PAYMENT P,USER U WHERE U.USER_ID=P.USER_ID AND P.TRANS_ID=B.TRANS_ID AND U.USER_NAME='${username}';`
    console.log(q);
...
module.exports.deleteTicket = function (pnr, Callback) {
    let query = `UPDATE BOARDING_PASS SET STATUS='Cancelled' where PNR_NUM=${pnr};`
    conn.query(query, (err) => {
...
module.exports.fromTo = function (Callback) {
    q = 'SELECT FROM_CITY,TO_CITY FROM FLIGHT;'
    conn.query(q, (err, rows) => {
        if (err) {
            console.log("Cannot fetch from to");
        }
        Callback(rows);
    })
}